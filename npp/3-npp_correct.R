...
#  of nearest cells within a given distance, with preference 
#  given to those in interpolation mask (within 9.3km of shore or reef).

# Output includes a final data raster and a 'flags' raster 
#   (0 = non-interpolated, 1 = interpolated with cells in interp,
#    2 = interpolated with cells outside interp)
...
interp_d <- 25000 # max. distance for interpolation (in meters)
ninterp_max <- 3 # max. number of cells to use for interpolation

# Read in the depth and interpolation masks
...
# Crop out extreme north (>83.5, since all cells with lat >83.22 are NA)
...
# For a point (long, lat) vector p, this function returns
#  a vector (interpolated value, flag)
get_interp <- function(p) {
    # Clip layers to rectangular buffer of at least interp_d around p
    ...
    # Prioritize using cells in interpolation mask, 
    # but if none, use any cell within interp_d 
    interp_idx <- which(interp_val == 1 & !is.na(prod_val))
    flag <- 1 # interpolated with close to shore/reef
    if (length(interp_idx) == 0) {
        interp_idx <- which(!is.na(prod_val))
        flag <- 2 # interpolated with other cells
    } 
    # If more than ninterp_max cells available, take the closest ones
...
interp_values <- t(apply(pts_interp, 1, get_interp))
